...
function main(): void {
  ...
  const consumerDir = fs.mkdtempSync(path.join(os.tmpdir(), "claude-code-to-llm-consumer-"));
  ...
  try {
    ...
    execFileSync(
      ...
      [
        "--input-type=module",
        "--eval",
        'const mod = await import("@yadimon/claude-code-to-llm"); if (typeof mod.runPrompt !== "function") throw new Error("missing runPrompt");'
      ],
      {
        cwd: consumerDir,
        env: sharedEnv,
