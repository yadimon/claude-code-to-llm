...
- `spawn.ts` — on Windows, extension-less commands are first upgraded to a real `.exe` when one exists (sibling probe for explicit paths, PATH probe for bare commands) and spawned directly. Remaining `.cmd`/`.bat` shims are wrapped through `cmd.exe /d /s /c` with cmd-specific quoting plus `windowsVerbatimArguments`; args containing newlines are rejected on that path because cmd truncates the command line at the first newline. Everything else is spawned directly. Keep platform-specific behavior explicit and testable.
